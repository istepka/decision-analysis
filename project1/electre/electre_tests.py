...
def test_marginal_concordance():
    test_alternatives = pd.DataFrame(
        [
            [90, 86, 46, 30],
            [40, 90, 14, 48],
            [94, 100, 40, 36],
    ...
    test_boundary_profiles = pd.DataFrame([[64, 61, 32, 32], [86, 84, 43, 43]]).astype(
        float
    )
    ...
    marginal_concordance_alt_to_profile, marginal_concordance_profile_to_alt = (
        marginal_concordance(
            test_alternatives,
            test_boundary_profiles,
            test_indifference_thresholds,
            test_preference_thresholds,
            test_gains,
        )
    )
    ...
    assert np.allclose(
        marginal_concordance_alt_to_profile[0],
        concordance_matrix_alt_to_profile,
        atol=0.01,
    )
    assert np.allclose(
        marginal_concordance_profile_to_alt[0],
        concordance_matrix_profile_to_alt,
        atol=0.01,
    )
...
def test_marginal_discordance():
    ...
    marginal_discordance_alt_to_profile, marginal_discordance_profile_to_alt = (
        marginal_discordance(
            test_alternatives,
            test_boundary_profiles,
            test_preference_thresholds,
            test_veto_thresholds,
            test_gains,
        )
    )

    assert np.allclose(
        marginal_discordance_alt_to_profile[1], discordance_alt_to_profile, atol=0.01
    )
    assert np.allclose(
        marginal_discordance_profile_to_alt[0], discordance_profile_to_alt, atol=0.01
    )
...
def test_comprehensive_concordance():
    ...
    (
        comprehensive_concordance_alt_to_profile,
        comprehensive_concordance_profile_to_alt,
...
def test_outranking_credibility():
    ...
    outrankings = outranking_credibility(
        comprehensive_concordance, marginal_discordance
    )
...
def test_transform_outranking_to_preference():
    ...
    assert np.allclose(
        outranking_preference[:, 0:1].flatten(),
        [[1, np.nan, 1, 1, -1, 0, 1, np.nan]],
        equal_nan=True,
    )
